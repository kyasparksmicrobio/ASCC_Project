...
{Now, we will figure out the 3 taxonomy strings with the highest relative abundance}
...
```{r}
# Step 1: Extract the site (TP, SF, SJ) from the sample names
# Assuming that the "TP", "SF", or "SJ" is always in a fixed position in the sample names
# We will create a new data frame to store the site information for each sample

# First, create a vector with sample column names (4-306)
sample_names <- colnames(Maaslin_shallow)[4:306]

# Extract the site code (TP, SF, SJ) from each sample name (adjust if needed for your pattern)
site_info <- sapply(sample_names, function(x) {
  stringr::str_extract(x, "TP|SF|SJ") # Extract site code
})

# Step 2: Add the site information to the feature table
# Create a data frame with relative abundance values and site info
feature_table_with_sites <- Maaslin_shallow %>%
  pivot_longer(cols = 4:306, names_to = "Sample", values_to = "Relative_Abundance") %>%
  mutate(Site2 = site_info[Sample])

# Step 3: Summarize the relative abundance by site for each taxonomy string
avg_abundance_by_site <- feature_table_with_sites %>%
  group_by(taxonomy_gtdb, Site2,OTUID) %>%
  summarize(Avg_Relative_Abundance = mean(Relative_Abundance, na.rm = TRUE))

# Step 4: Identify the site with the highest relative abundance for each taxonomy string
max_abundance_by_taxonomy <- avg_abundance_by_site %>%
  group_by(taxonomy_gtdb, OTUID) %>%
  slice_max(order_by = Avg_Relative_Abundance, n = 1)

# View the final result showing the site with the highest abundance
print(max_abundance_by_taxonomy)
```

```{r}
write.csv(max_abundance_by_taxonomy, "/Users/kyasparks/Desktop/Kyas_PhD/CSU_2023_2024/Projects/ASCC/ASCC_July2024_USETHIS_ks/16S/16S_Maaslin/ASCC_16S_maxRA_shallow.csv")

write.csv(avg_abundance_by_site, "/Users/kyasparks/Desktop/Kyas_PhD/CSU_2023_2024/Projects/ASCC/ASCC_July2024_USETHIS_ks/16S/16S_Maaslin/ASCC_16S_avgRAbysite_shallow.csv")
```
...
# First, create a vector with sample column names (4-306)
sample_names <- colnames(Maaslin_deep)[4:306]
...
# Step 2: Add the site information to the feature table
# Create a data frame with relative abundance values and site info
feature_table_with_sites <- Maaslin_deep %>%
  pivot_longer(cols = 4:306, names_to = "Sample", values_to = "Relative_Abundance") %>%
  mutate(Site2 = site_info[Sample])
...
```{r}
write.csv(max_abundance_by_taxonomy, "/Users/kyasparks/Desktop/Kyas_PhD/CSU_2023_2024/Projects/ASCC/ASCC_July2024_USETHIS_ks/16S/16S_Maaslin/ASCC_16S_maxRA_deep.csv")

write.csv(avg_abundance_by_site, "/Users/kyasparks/Desktop/Kyas_PhD/CSU_2023_2024/Projects/ASCC/ASCC_July2024_USETHIS_ks/16S/16S_Maaslin/ASCC_16S_avgRAbysite_deep.csv")
```
...
# First, create a vector with sample column names (4-306)
sample_names <- colnames(Maaslin_OM)[4:306]
...
# Step 2: Add the site information to the feature table
# Create a data frame with relative abundance values and site info
feature_table_with_sites <- Maaslin_OM %>%
  pivot_longer(cols = 4:306, names_to = "Sample", values_to = "Relative_Abundance") %>%
  mutate(Site2 = site_info[Sample])
...
```{r}
write.csv(max_abundance_by_taxonomy, "/Users/kyasparks/Desktop/Kyas_PhD/CSU_2023_2024/Projects/ASCC/ASCC_July2024_USETHIS_ks/16S/16S_Maaslin/ASCC_16S_maxRA_OM.csv")

write.csv(avg_abundance_by_site, "/Users/kyasparks/Desktop/Kyas_PhD/CSU_2023_2024/Projects/ASCC/ASCC_July2024_USETHIS_ks/16S/16S_Maaslin/ASCC_16S_avgRAbysite_OM.csv")
```

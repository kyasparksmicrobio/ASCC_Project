...
# ✅ Read in discriminant results from SF5_discrim
...
# ✅ Read in discriminant results from TP5_discrim
...
# ✅ Ensure OTUID formatting matches
...
# ✅ Count how many OTUs are now discriminant for SF & TP
...
data <- data %>%
  mutate(
    ...
    # ✅ Correct enrichment classification
...
plot <- ggplot(data, aes(x = coef, y = log_q)) +
  ...
  # 🔥 Highlight SF-enriched OTUs (black outline) → Right side
  ...
  geom_point(data = data %>% filter(Highlight == "Discriminant for TP" & coef > 0),
           aes(x = coef, y = log_q, fill = Enrichment),
           shape = 21, size = 3, stroke = 0.6, color = "red") +   # ✅ FIXED COLOR
  
  # 🔴 Highlight TP-enriched OTUs (red outline) → Left side only
  geom_point(data = data %>% filter(Highlight == "Discriminant for TP" & coef < 0),
             aes(x = coef, y = log_q, fill = Enrichment),
             shape = 21, size = 3, stroke = 0.6, color = "red") +   # ✅ FIXED COLOR
...
scale_fill_manual(
  ...
  na.translate = FALSE,
  name = "Top 10 OTUs")+
  # ✅ Color scheme: Non-Significant → Grey
  scale_color_manual(values = c(
    "SF Enriched" = "#B75347",  # ✅ Red for SF
    "TP Enriched" = "#94B594",  # ✅ Green for TP
...
plot <- ggplot(data, aes(x = coef, y = log_q)) +
  ...
  scale_color_manual(values = c(
    "SF Enriched" = "#B75347",  # ✅ Red for SF
    "SJ Enriched" = "#94B594",  # ✅ Green for TP
    "Non-Significant" = "#B0B0B0"
  )) +
...
# ✅ Identify SF-enriched OTUs (SF higher than TP & SJ)
discriminant_TP <- discriminant_data_tp %>%
  filter(coef.x > 0 & coef.y > 0) %>%   # ✅ FIXED
  select(OTUID) %>%
  pull()

# ✅ Identify TP-enriched OTUs (TP higher than SF & SJ) → **Only appear on the left side**
discriminant_SJ <- discriminant_data_sj %>%
  filter(coef.x < 0 & coef.y < 0) %>%   # ✅ FIXED (TP is enriched over SF & SJ)
  select(OTUID) %>%
  pull()
...
# Extract OTUIDs
discriminant_SF <- trimws(as.character(discriminant_data_sf$OTUID))
discriminant_SJ <- trimws(as.character(discriminant_data_sj$OTUID))

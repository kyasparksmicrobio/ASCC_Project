maaslin <- read_excel("/Users/kyasparks/Desktop/Desktop - MacBook Pro (2)/Kyas_PhD/CSU_2023_2024/Projects/ASCC/ASCC_July2024_USETHIS_ks/16S/16S_Maaslin/ASCC_16S_Maaslin2_finalresultscombined.xlsx",sheet = "all")
...
ggplot(data_meta_taxa, aes(x = Proportion, y = Phylum, color = Site)) +
  ...
  facet_wrap(~ Depth, ncol = 1)  # Customize colors for 3 sites
...
maaslin.1 <- read_excel("/Users/kyasparks/Desktop/Desktop - MacBook Pro (2)/Kyas_PhD/CSU_2023_2024/Projects/ASCC/ASCC_July2024_USETHIS_ks/16S/16S_Maaslin/ASCC_16S_Maaslin2_finalresultscombined.xlsx",sheet = "shallow")

maaslin.2 <- read_excel("/Users/kyasparks/Desktop/Desktop - MacBook Pro (2)/Kyas_PhD/CSU_2023_2024/Projects/ASCC/ASCC_July2024_USETHIS_ks/16S/16S_Maaslin/ASCC_16S_Maaslin2_finalresultscombined.xlsx",sheet = "deep")

maaslin.3 <- read_excel("/Users/kyasparks/Desktop/Desktop - MacBook Pro (2)/Kyas_PhD/CSU_2023_2024/Projects/ASCC/ASCC_July2024_USETHIS_ks/16S/16S_Maaslin/ASCC_16S_Maaslin2_finalresultscombined.xlsx",sheet = "OM")

# # Step 1: Identify Phyla Present in Every Site
phylum_site_count <- maaslin.1 %>%
  group_by(Phylum) %>%
  summarise(Site_Count = n_distinct(Site)) %>%
  filter(Site_Count == length(unique(maaslin.1$Site))) %>%
  pull(Phylum)

# # Step 2: Subset the 'maaslin' Dataset
maaslin_subset <- maaslin.1 %>%
  filter(Phylum %in% phylum_site_count)

# Count phylum occurrences grouped by site
phylum_counts <- maaslin.1%>%
  group_by(Site, Phylum) %>%
  summarise(Phylum_Count = n()) %>%
  ungroup()

...
all.equal(row.names(meta_shallow),sample_names(physeq)) # Final check to make sure everything lines up
sampledata<-sample_data(meta_shallow)
...
#### this is to check for missing samples between files.
## IGNORE UNLESS NEEDED
# sample_names_meta <- meta_shallow %>% pull(Name)

# column_names_otus <- colnames(otus_shallow)

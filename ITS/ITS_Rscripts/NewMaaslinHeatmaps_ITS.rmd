...
maaslin_sf_5 <- read_excel("/Users/kyasparks/Desktop/CSU 2023-2028/Projects/ASCC/ASCC_July2024_USETHIS_ks/ITS/ITS_maaslin/Maaslin_ITS_discrimcomparisons_040725.xlsx", sheet = "SF5_discrim") %>%
  mutate(Depth = "0-5cm")

maaslin_sf_15 <- read_excel("/Users/kyasparks/Desktop/CSU 2023-2028/Projects/ASCC/ASCC_July2024_USETHIS_ks/ITS/ITS_maaslin/Maaslin_ITS_discrimcomparisons_040725.xlsx", sheet = "SF15_discrim") %>%
  mutate(Depth = "5-15cm")

maaslin_sf_OM <- read_excel("/Users/kyasparks/Desktop/CSU 2023-2028/Projects/ASCC/ASCC_July2024_USETHIS_ks/ITS/ITS_maaslin/Maaslin_ITS_discrimcomparisons_040725.xlsx", sheet = "SFOM_discrim") %>%
  mutate(Depth = "OM")

# Step 2: Combine and clean taxonomy
maaslin_all <- bind_rows(maaslin_sf_5, maaslin_sf_15, maaslin_sf_OM) %>%
  separate(taxonomy.x, into = c("Domain", "Phyla", "Class", "Order", "Family", "Genus", "Species"), sep = ";") %>%
...
# Store phylum levels from most to least OTUs
phylum_order <- phylum_totals$Phyla
...
expand_phylum_grid <- function(data, all_phyla, phylum_order) {
  ...
  data$Phyla <- factor(data$Phyla, levels = rev(phylum_order))  # 💥 Reverse order here
...
shared_theme <- theme_minimal(base_size = 13) +
  theme(
    ...
    axis.text.x.top = element_text(angle = 0, hjust = 0.5),  # <-- fixed here
...
expand_phylum_grid <- function(data, all_phyla, phylum_order) {
  ...
  expanded <- expand.grid(Phyla = all_phyla, Depth = depths, KEEP.OUT.ATTRS = FALSE, stringsAsFactors = FALSE)
  data <- full_join(expanded, data, by = c("Phyla", "Depth"))
  data$Phyla <- factor(data$Phyla, levels = rev(phylum_order))
  data$Depth <- factor(data$Depth, levels = depths)
  data$fill_val[is.na(data$fill_val)] <- NA
  data$n_OTUs[is.na(data$n_OTUs)] <- 0
  data$label[is.na(data$label)] <- "0"
